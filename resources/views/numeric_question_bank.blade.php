...
{{-- Page title --}}
@section('title')
    Numeric @if($status==1) Active  @else Inactive @endif Item Bank
    @parent
@stop
...
@section('content')
    <section class="content-header">
        <!--section starts-->
        <h1>Numeric @if($status==1) Active  @else Inactive @endif Item Bank</h1>
        <ol class="breadcrumb">
            <li>
                <a href="#">Admin</a>
            </li>
            ...
            @endif
            <li class="active">Numeric @if($status==1) Active  @else Inactive @endif Item Bank</li>
        </ol>
    </section>
                ...
                <div class="panel panel-info">
                    <div class="panel-heading clearfix">
                        ...
                        <div class="pull-right">
                            <a  href="{{ URL::to('/create-numeric-question') }}" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-plus"></span>Add Numeric Item</a>
                        </div>
                    </div>
                            ...
                            <tbody>
                                @foreach($numeric_questions as $key => $value)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ URL::to('/update-numeric-question/'.$value->id.'/'.$status) }}"><i class="livicon" data-name="edit" data-size="20" data-loop="true" data-c="#F89A14" data-hc="#F89A14" title="Update data" ></i></a>
    ...
    <script>
        $(document).ready(function() {
            ...
            if (sessionStorage.getItem('update_success') == 'success') {
                ...
                toastr.success('Numeric item has been successfully updated', 'Success Alert', {timeOut: 5000});
                sessionStorage.removeItem("update_success");
            } else if (sessionStorage.getItem('new_success') == 'success')
            {
                toastr.success('Numeric item has been successfully created', 'Success Alert', {timeOut: 5000});
