$(document).ready(function(){
    ...
    $('#sub_question').on('click', function(){
        ...
        $('#more_sub_question').append('<div class="sub_ques">'+
            '<span class="btn btn-danger btn-sm remove_sub_question" style="float: right;margin-bottom: 2px;">&times;</span>' +
            '<div class="form-group">'+
            '<label for="sub_question">Sub Question</label>' +
            '<div class="form-group">'+
            '<div class="fileinput fileinput-new" data-provides="fileinput">'+
            '<div class="fileinput-new thumbnail">'+
            '<img src="../assets/img/authors/no_avatar.jpg" alt="..." class="img-responsive" style="width: 100px; height: 100px;"/>'+
            '</div>'+
    ...
    $(document).on('click', '.text_opt', function(e){
        $(this).prev('.more_text_opt').append('<div class="form-group extra_field">' +
            ...
            '<input type="text" class="form-control text_options" name="text_options_0[]" placeholder="Option" required/>' +
            '<label class="invalid-feedback">This field is required.</label>' +
            '</div>'
        );

        var selectList  = $(this).next(".sub_right_answer");
        selectList.find('option').remove();

        var numItems = $('.text_options').length;

        $(this).next(".sub_right_answer").append("<option value=''> Choose Sub Correct Answer </option>");
        for (var i=1; i<=numItems; i++)
        {
            $(this).next(".sub_right_answer").append("<option value="+ i +">Option "+ i +"</option>")
        }
    });
    ...
    $(document).on('click', '.remove', function(e){
        ...
        var selectList  = $(".sub_right_answer");
        selectList.find('option').remove();

        var numItems = $('.text_options').length;

        $(".sub_right_answer").append("<option value=''> Choose Sub Correct Answer </option>");
        for (var i=1; i<=numItems; i++)
        {
            $(".sub_right_answer").append("<option value="+ i +">Option "+ i +"</option>")
        }
    });
});
